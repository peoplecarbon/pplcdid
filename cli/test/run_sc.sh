...
# export PPLDIDCMD='pplcdid'

# CONTAINER_NAME=test_sc_1
export CONTAINER_NAME=pplcdid
...
# $PPLDIDCMD sc_init -l $SEMCON_URL --token $ADMIN_TOKEN --doc-key c2/private_key.b58 --rev-key c2/revocation_key.b58

# # create DID for Semantic Container
SC_DID=`$PPLDIDCMD sc_init -l $SEMCON_URL --token $ADMIN_TOKEN --doc-key c2/private_key.b58 --rev-key c2/revocation_key.b58 |
    jq -r '.did'`
echo "DID: $SC_DID"

# writing to Semantic Container
TOKEN=`$PPLDIDCMD sc_token $SC_DID --doc-key c2/private_key.b58 | \
    jq -r '.access_token'`
echo '{"hello": "world"}' | \
...
cat c2/data.json | $PPLDIDCMD sc_create $SC_DID --doc-pwd=pwd3 --rev-pwd=pwd4 --token $ADMIN_TOKEN

# # output DID
# $PPLDIDCMD read --w3c-did "$DID_REC;$SEMCON_URL"