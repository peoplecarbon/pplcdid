...
def ppld_decode(message)
    Multibases.unpack(message).decode.to_s('ASCII-8BIT')
end

def ppld_hash(message)
    ppld_encode(Multihashes.encode(Digest::SHA256.digest(message), "sha2-256").unpack('C*'))
end
...
def add_hash(log)
    log.map do |item|
        i = item.dup
        i.delete("previous")
        item["entry-hash"] = ppld_hash(item.to_json)
        if item["op"] == 1
            item["sub-entry-hash"] = ppld_hash(i.to_json)
        end
        item
    end
end
...
def match_log_did?(log, doc)
    ...
    pubKey = Ed25519::VerifyKey.new(ppld_decode(pubKey_string))
    signature = ppld_decode(log["sig"])
    begin
...
def get_key(filename, key_type)
    ...
    if key_type == "sign"
        return Ed25519::SigningKey.new(ppld_decode(key_encoded))
    else
        return Ed25519::VerifyKey.new(ppld_decode(key_encoded))
    end
end
...
def delete_did(did, options)
    ...
    did_data = {
        "dockey": ppld_encode(privateKey.to_bytes),
        "revkey": ppld_encode(revocationKey.to_bytes)
    }
...
def write_did(content, did, mode, options)
    ...
    if mode == "create" || mode == "clone"
        ...
        if options[:rev_key].nil? && options[:rev_pwd].nil?
            ...
            did_key = ppld_encode(publicKey.to_bytes) + ":" + ppld_encode(pubRevoKey.to_bytes)
            subDid = {"doc": did_old_doc, "key": did_key}.to_json
            subDidHash = ppld_hash(subDid)
            signedSubDidHash = ppld_encode(revocationKey.sign(subDidHash))
            revocationLog = { 
                "ts": ts_old,
                "op": 1, # REVOKE
                "doc": subDidHash,
        ...
        revoc_log["previous"] = [
            ppld_hash(old_log[did_info["doc_log_id"].to_i].to_json), 
            ppld_hash(old_log[did_info["termination_log_id"].to_i].to_json)
        ]
        prev_hash = [ppld_hash(revoc_log.to_json)]
    end
    ...
    #   signature = [ppld_decode(signedSubDidHash).to_s(16)].pack('H*')
    #   message = subDidHash
    #   pubRevoKey.verify(signature, message)

    # build termination log entry
    l2_doc = ppld_hash(r1.to_json)
    if !doc_location.nil?
    ...
    l2 = { "ts": ts,
           "op": 0, # TERMINATE
           "doc": l2_doc,
           "sig": ppld_encode(privateKey.sign(l2_doc)),
           "previous": [] }.transform_keys(&:to_s)

    # build actual DID document
    log_str = ppld_hash(l2.to_json)
    if !doc_location.nil?
    ...
    # create DID
    l1_doc = ppld_hash(didDocument.to_json)
    if !doc_location.nil?
    ...
    if mode == "clone"
        ...
        new_log = {
            ...
            "op": 4, # CLONE
            "doc": l1_doc,
            "sig": ppld_encode(privateKey.sign(l1_doc)),
        ...
        prev_hash = [ppld_hash(new_log.to_json)]
    end
    ...
    when /^http/
        ...
        if options[:doc_pwd].nil? && options[:doc_key].nil?
            File.write(did10 + "_private_key.b58", ppld_encode(privateKey.to_bytes))
        end
        if options[:rev_pwd].nil? && options[:rev_key].nil?
            File.write(did10 + "_revocation_key.b58", ppld_encode(revocationKey.to_bytes))
...
def clone_did(did, options)
    ...
    options[:previous_clone] = ppld_hash(source_log) + LOCATION_PREFIX + source_location
...
def sc_token(did, options)
    ...
    did_info = resolve_did(did, options)
    if did_info["doc"]["key"].split(":")[0].to_s != ppld_encode(privateKey.verify_key.to_bytes)
        if options[:silent].nil? || !options[:silent]
    ...
    response = HTTParty.post(init_url,
        headers: { 'Content-Type' => 'application/json' },
        body: { "session_id": sid, 
                "public_key": ppld_encode(privateKey.verify_key.to_bytes) }.to_json ).parsed_response rescue {}
    ...
    response = HTTParty.post(token_url,
        headers: { 'Content-Type' => 'application/json' },
        body: { "session_id": sid, 
                "signed_challenge": ppld_encode(privateKey.sign(challenge)) }.to_json).parsed_response rescue {}
