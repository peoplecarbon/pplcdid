class DidsController < ApplicationController
    ...
    def create
        ...
        if didHash != Pplcdid.hash(Pplcdid.canonical(didDocument))
        ...
        logs.each do |item|
            if item["op"] == 0 # TERMINATE
                log_entry_hash = Pplcdid.hash(Pplcdid.canonical(item))
            end
        end
        if log_entry_hash != ""
    ...
    def delete
        ...
        if public_doc_key == Pplcdid.public_key(private_doc_key).first &&
           public_rev_key == Pplcdid.public_key(private_rev_key).first
                Log.where(did: params[:did].to_s).destroy_all
                Did.where(did: params[:did].to_s).destroy_all
                render plain: "",
                       status: 200
        else
            puts "Doc key: " + public_doc_key.to_s + " <=> " + Pplcdid.public_key(private_doc_key).first.to_s
            puts "Rev key: " + public_rev_key.to_s + " <=> " + Pplcdid.public_key(private_rev_key).first.to_s
    ...
    def uniregistrar_create
        ...
        if !did_obj.nil? && did_obj.is_a?(Hash)
            if did_obj["@context"] == "https://www.w3.org/ns/did/v1"
                doc = Pplcdid.fromW3C(didDocument, options)
            end
        end
        ...
        if !did_obj.nil? && did_obj.is_a?(Hash)
            if did_obj["doc"].to_s != "" && did_obj["key"].to_s != "" && did_obj["log"].to_s != ""
                if !options[:log_create].nil? && !options[:log_terminate].nil?
                    ...
                    if Pplcdid.hash(Pplcdid.canonical(did_obj)) != options[:log_create]["doc"]
                    ...
                    success, msg = Pplcdid.verify(options[:log_create]["doc"], options[:log_create]["sig"], doc_pubkey)
                    if !success
                    ...
                    success, msg = Pplcdid.verify(options[:log_terminate]["doc"], options[:log_terminate]["sig"], doc_pubkey)
                    if !success
                    ...
                    # create DID
                    did = "did:pplc:" + Pplcdid.hash(Pplcdid.canonical(did_obj))
                    logs = [options[:log_create], options[:log_terminate]]
                    success, msg = Pplcdid.publish(did, did_obj, logs, options)
                    if success
                        w3c_input = {
                            "did" => did,
                            "doc" => didDocument
                        ...
                        status = {
                            "did" => did,
                            "doc" => didDocument,
                            "doc_w3c" => Pplcdid.w3c(w3c_input, options),
                            "log" => logs,
                            "private_key" => "",
                            "revocation_key" => "",
                            "revocation_log" => []
        ...
        if !preprocessed
            status, msg = Pplcdid.create(doc, options)
        end
        if status.nil?
            ...
            retVal = {
                "didState": {
                    "did": Pplcdid.percent_encode(status["did"]),
                    "state": "finished",
                    "secret": {
                ...
                "didDocumentMetadata": {
                    "did": Pplcdid.percent_encode(status["did"]),
                    "registry": Pplcdid.get_location(status["did"].to_s),
    ...
    def uniregistrar_update
        ...
        if !did_obj.nil? && did_obj.is_a?(Hash)
            if did_obj["@context"] == "https://www.w3.org/ns/did/v1"
                did_obj = Pplcdid.fromW3C(did_obj, options)
            end
        end
        ...
        if !did_obj.nil? && did_obj.is_a?(Hash)
            if !options[:log_revoke].nil? && !options[:log_update].nil? && !options[:log_terminate].nil?
                ...
                doc_pubkey = did_obj["key"].split(":").first.to_s
                old_doc_location = Pplcdid.get_location(old_did)
                old_didDocument = Pplcdid.retrieve_document_raw(old_did, "", old_doc_location, {})
                old_doc_pubkey = old_didDocument.first["doc"]["key"].split(":").first.to_s
                success, msg = Pplcdid.verify(options[:log_update]["doc"], options[:log_update]["sig"], old_doc_pubkey)
                if !success
        ...
        if !preprocessed
            status, msg = Pplcdid.update(did_obj, old_did, options)
        end
        if status.nil?
            render json: {"error": msg},
                   status: 500
    ...
    def uniregistrar_deactivate
        ...
        if !preprocessed
            status, msg = Pplcdid.revoke(did, options)
        end
        ...
        if status.nil?
            ...
            retVal = {
                "didState": {
                    "did": Pplcdid.percent_encode(did),
                    "state": "finished",
                },
                "didRegistrationMetadata": {},
                "didDocumentMetadata": {
