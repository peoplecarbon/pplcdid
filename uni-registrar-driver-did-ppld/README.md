...
docker run -p 8080:3000 peoplecarbon/ppld-registrar
```
...
```bash
echo '{"didDocument":{"@context":"https://www.w3.org/ns/did/v1", "authentication":[], "service":[{"privacy":"https://data-vault.eu/api/data"}]}}' | curl -H "Content-Type: application/json" -d @- -X POST https://ppld-registrar.peoplecarbon.org/1.0/create
```

DID: [`did:pplc:zQmS3dpwUAJeozNgtAn4E5BT8mk9quGwe92Jrh4MSJMXrAa`](https://ppld-resolver.peoplecarbon.org/1.0/identifiers/did:pplc:zQmS3dpwUAJeozNgtAn4E5BT8mk9quGwe92Jrh4MSJMXrAa)
...
The JSON structure above can be generated by using the `--simulate` option of the [PPLDID CLI](https://github.com/peoplecarbon/pplcdid/tree/main/cli). Use the following command:
...
DID: [`did:pplc:zQmSU6NU1e4tzSP8ti8vAJfUpYh1zL2RASuKJAd1BwT667U`](https://ppld-resolver.data-container.net/1.0/identifiers/did:pplc:zQmSU6NU1e4tzSP8ti8vAJfUpYh1zL2RASuKJAd1BwT667U)
...
The JSON structure from above to invoke `POST /1.0/update` can then be generated by using the `--simulate` option of the [PPLDID CLI](https://github.com/peoplecarbon/pplcdid/tree/main/cli) with the following command:
...
Attempting to resolve this DID will result in a "DID not found" response: [`did:pplc:zQmPyjnkL52gQxBBhPuCFkf167MzpCKquqbu5Qt5ia2JGTr`](https://ppld-resolver.data-container.net/1.0/identifiers/did:pplc:zQmPyjnkL52gQxBBhPuCFkf167MzpCKquqbu5Qt5ia2JGTr)

*Note:* with the `logs` option of the [PPLDID CLI](https://github.com/peoplecarbon/pplcdid/tree/main/cli) it is possible to show the the complete log trail for a deactivated DID:
...
The JSON structure from above to invoke `POST /1.0/deactivate` can then be generated by using the `--simulate` option of the [PPLDID CLI](https://github.com/peoplecarbon/pplcdid/tree/main/cli) with the following command:
