...
```
docker build -f ./docker/Dockerfile . --tag peoplecarbon/pplcid-registrar:<version>
docker run -p 8080:3000 peoplecarbon/pplc-registrar
```

Docker images are available here: https://hub.docker.com/r/peoplecarbon/pplcid-registrar

### Verify with automated tests    

Use the following command to run the automated tests (including pplcid-registrar tests) in the [`peoplecarbon/pplcid-cli`](https://hub.docker.com/r/peoplecarbon/pplcid-cli) Docker image:    

```bash
docker run -it --rm -w /usr/src/pytest -e PPLDIDCMD=pplcid peoplecarbon/pplc-cli pytest
```
...
The JSON structure above can be generated by using the `--simulate` option of the [PPLCID CLI](https://github.com/peoplecarbon/pplcid/tree/main/cli). Use the following command:
...
The JSON structure from above to invoke `POST /1.0/update` can then be generated by using the `--simulate` option of the [PPLCID CLI](https://github.com/peoplecarbon/pplcid/tree/main/cli) with the following command:
...
*Note:* with the `logs` option of the [PPLCID CLI](https://github.com/peoplecarbon/pplcid/tree/main/cli) it is possible to show the the complete log trail for a deactivated DID:
...
The JSON structure from above to invoke `POST /1.0/deactivate` can then be generated by using the `--simulate` option of the [PPLCID CLI](https://github.com/peoplecarbon/pplcid/tree/main/cli) with the following command:
