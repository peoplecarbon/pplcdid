# Universal Registrar Driver: `did:ppld`

This is a [Universal Registrar](https://github.com/decentralized-identity/universal-registrar/) driver for **did:ppld** identifiers. 
...
```
docker build -f ./docker/Dockerfile . --tag peopledataorg/ppldid-registrar:<version>
docker run -p 8080:3000 peopledataorg/ppld-registrar
```

Docker images are available here: https://hub.docker.com/r/peopledata/ppldid-registrar

### Verify with automated tests    

Use the following command to run the automated tests (including ppldid-registrar tests) in the [`peopledata/ppldid-cli`](https://hub.docker.com/r/peopledata/ppldid-cli) Docker image:    
...
* or by sumitting the raw PPLDID data (i.e., PPLDID Document and PPLDID Logs) that does not require to disclose private keys to Uniregistrar.    
...
```bash
echo '{"didDocument":{"@context":"https://www.w3.org/ns/did/v1", "authentication":[], "service":[{"privacy":"https://data-vault.eu/api/data"}]}}' | curl -H "Content-Type: application/json" -d @- -X POST https://ppld-registrar.peopledata.org.cn/1.0/create
```

DID: [`did:ppld:zQmS3dpwUAJeozNgtAn4E5BT8mk9quGwe92Jrh4MSJMXrAa`](https://ppld-resolver.peopledata.org.cn/1.0/identifiers/did:ppld:zQmS3dpwUAJeozNgtAn4E5BT8mk9quGwe92Jrh4MSJMXrAa)
...
**PPLDID Internal Format without Private Keys**    
...
The JSON structure above can be generated by using the `--simulate` option of the [PPLDID CLI](https://github.com/peopledata/ppldid/tree/main/cli). Use the following command:

```bash
echo '{"privacy": "https://data-vault.eu/api/data"}' | \
  ppldid create --simulate --doc-pwd myDocPwd --rev-pwd MyRevocationPwd | \
...
The update operation requires as input the DID that should be updated and for PPLDID to publish the private revocation record generated when building the original DID. This revocation record can also be re-built using the orginal private keys provided in `secret`.
...
```json
{
  "identifier": "did:ppld:zQmPy6LnarRoWyXau5DaL1NX8wn7QWzigx6r8qNUvVF4DgZ",
  "secret": {
...
```bash
echo '{"identifier":"did:ppld:zQmPy6LnarRoWyXau5DaL1NX8wn7QWzigx6r8qNUvVF4DgZ","secret":{"old_doc_pwd":"myDocPwd","old_rev_pwd":"myRevPwd","doc_pwd":"newDocPwd","rev_pwd":"myRevPwd"},"didDocument":{"@context":"https://www.w3.org/ns/did/v1","service":[{"state":"new"}]}}' | \
    curl -H "Content-Type: application/json" -d @- -X POST https://ppldid-registrar.data-container.net/1.0/update
```

DID: [`did:ppld:zQmSU6NU1e4tzSP8ti8vAJfUpYh1zL2RASuKJAd1BwT667U`](https://ppld-resolver.data-container.net/1.0/identifiers/did:ppld:zQmSU6NU1e4tzSP8ti8vAJfUpYh1zL2RASuKJAd1BwT667U)
...
When updating an existing DID Document it is also possible to provide the new version (in `didDocument`) and the relevant PPLDID log entries (in `options`).

`POST /1.0/update`

```json
{
  "identifer": "did:ppld:zQmRgH1oAC6ucvg69ewtTZVaqcUNzx7dasF7nrwonzWhxM4",
  "options": {
    "log_revoke": {
      "ts": 1647444355,
      "op": 2,
...
```bash
echo '{"state": "initial"}' | ppldid create --doc-pwd pwd1 --rev-pwd pwd2
```

Output: `did:ppld:zQmfTxojQjja2gVvnuGEHyWZU6zQAdLLErMt3T8Wei7MfFX`

The JSON structure from above to invoke `POST /1.0/update` can then be generated by using the `--simulate` option of the [PPLDID CLI](https://github.com/peopledata/ppldid/tree/main/cli) with the following command:

```bash
echo '{"state": "new"}' | \
    ppldid update did:ppld:zQmfTxojQjja2gVvnuGEHyWZU6zQAdLLErMt3T8Wei7MfFX \
...
```bash
did_v1=`echo '{"state": "initial"}' | ppldid create --doc-pwd pwd1 --rev-pwd pwd2 --json-output`
echo '{"state": "new"}' | ppldid update $(echo $did_v1 | jq -r '.did') --simulate --old-doc-pwd pwd1 --old-rev-pwd pwd2 --doc-pwd pwd3 --rev-pwd pwd4 | jq '{identifier: .did_old, options: {log_revoke: .log_revoke_old, log_update: .log_update, log_terminate: .log_terminate}, didDocument: .doc}' | curl -H "Content-Type: application/json" -d @- -X POST http://localhost:3000/1.0/update
```
...
The deactivate operation in PPLDID consists of publishing the associated revocation record. It requires as input the DID that should be revoked and either the private keys or the revocation log record itself.
...
```json
{
  "identifier": "did:ppld:zQmPyjnkL52gQxBBhPuCFkf167MzpCKquqbu5Qt5ia2JGTr",
  "secret": {
    "doc_pwd": "pwd1",
    "rev_pwd": "pwd2"
  }
...
```bash
echo '{"identifier":"did:ppld:zQmPyjnkL52gQxBBhPuCFkf167MzpCKquqbu5Qt5ia2JGTr","secret":{"doc_pwd":"pwd1","rev_pwd":"pwd2"}}' | \
    curl -H "Content-Type: application/json" -d @- -X POST https://ppldid-registrar.data-container.net/1.0/deactivate
```

Attempting to resolve this DID will result in a "DID not found" response: [`did:ppld:zQmPyjnkL52gQxBBhPuCFkf167MzpCKquqbu5Qt5ia2JGTr`](https://ppld-resolver.data-container.net/1.0/identifiers/did:ppld:zQmPyjnkL52gQxBBhPuCFkf167MzpCKquqbu5Qt5ia2JGTr)

*Note:* with the `logs` option of the [PPLDID CLI](https://github.com/peopledata/ppldid/tree/main/cli) it is possible to show the the complete log trail for a deactivated DID:

```bash
ppldid logs did:ppld:zQmPyjnkL52gQxBBhPuCFkf167MzpCKquqbu5Qt5ia2JGTr
```
...
```json
{
  "identifer": "did:ppld:zQmQCoR1HMEBFuc1dzaNpGqMcbpf5zAnJGq8tmoQFrTB4PZ",
  "options": {
    "log_revoke": {
      "ts": 1647444355,
      "op": 2,
...
```bash
echo '{"short": "lived"}' | ppldid create --doc-pwd pwd1 --rev-pwd pwd2
```

Output: `did:ppld:zQmPbLw9vJzjNHHDNqpnK4rFjisUeXhLFvy7Apm6yWEPKt9`

The JSON structure from above to invoke `POST /1.0/deactivate` can then be generated by using the `--simulate` option of the [PPLDID CLI](https://github.com/peopledata/ppldid/tree/main/cli) with the following command:

```bash
ppldid revoke did:ppld:zQmfTxojQjja2gVvnuGEHyWZU6zQAdLLErMt3T8Wei7MfFX \
...
```bash
echo '{"short": "lived"}' | \
    ppldid create --doc-pwd pwd1 --rev-pwd pwd2 --json-output | \
...
* `log_create`: **create PPLDID log entry** (JSON object) - entry for the PPLDID log that contains information for creating a DID    
* `log_update`: **update PPLDID log entry** (JSON object) - entry for the PPLDID log that contains information for updating a DID    
* `log_terminate`: **terminate PPLDID log entry** (JSON object) - entry for the PPLDID log that contains information to determine if their exists an update for the current DID    
* `log_revoke`: **revoke PPLDID log entry** (JSON object) - entry for the PPLDID log that contains information that the associated termination entry does not indiciate that the resolution process is complete    
* `log_revoke_old`: **revoke PPLDID log entry from previous DID version** (JSON object) - entry for the PPLDID log that contains information about the previous DID version    
