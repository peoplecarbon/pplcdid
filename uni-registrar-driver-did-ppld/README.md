...
```
docker build -f ./docker/Dockerfile . --tag peopledataorg/pplcid-registrar:<version>
docker run -p 8080:3000 peopledataorg/pplc-registrar
```

Docker images are available here: https://hub.docker.com/r/peopledata/pplcid-registrar

### Verify with automated tests    

Use the following command to run the automated tests (including pplcid-registrar tests) in the [`peopledata/pplcid-cli`](https://hub.docker.com/r/peopledata/pplcid-cli) Docker image:    
...
```bash
echo '{"didDocument":{"@context":"https://www.w3.org/ns/did/v1", "authentication":[], "service":[{"privacy":"https://data-vault.eu/api/data"}]}}' | curl -H "Content-Type: application/json" -d @- -X POST https://pplc-registrar.peopledata.org.cn/1.0/create
```

DID: [`did:pplc:zQmS3dpwUAJeozNgtAn4E5BT8mk9quGwe92Jrh4MSJMXrAa`](https://pplc-resolver.peopledata.org.cn/1.0/identifiers/did:pplc:zQmS3dpwUAJeozNgtAn4E5BT8mk9quGwe92Jrh4MSJMXrAa)
...
The JSON structure above can be generated by using the `--simulate` option of the [PPLDID CLI](https://github.com/peopledata/pplcid/tree/main/cli). Use the following command:

```bash
echo '{"privacy": "https://data-vault.eu/api/data"}' | \
  pplcid create --simulate --doc-pwd myDocPwd --rev-pwd MyRevocationPwd | \
...
echo '{"identifier":"did:pplc:zQmPy6LnarRoWyXau5DaL1NX8wn7QWzigx6r8qNUvVF4DgZ","secret":{"old_doc_pwd":"myDocPwd","old_rev_pwd":"myRevPwd","doc_pwd":"newDocPwd","rev_pwd":"myRevPwd"},"didDocument":{"@context":"https://www.w3.org/ns/did/v1","service":[{"state":"new"}]}}' | \
    curl -H "Content-Type: application/json" -d @- -X POST https://pplcid-registrar.data-container.net/1.0/update
```
...
```bash
echo '{"state": "initial"}' | pplcid create --doc-pwd pwd1 --rev-pwd pwd2
```

Output: `did:pplc:zQmfTxojQjja2gVvnuGEHyWZU6zQAdLLErMt3T8Wei7MfFX`

The JSON structure from above to invoke `POST /1.0/update` can then be generated by using the `--simulate` option of the [PPLDID CLI](https://github.com/peopledata/pplcid/tree/main/cli) with the following command:

```bash
echo '{"state": "new"}' | \
    pplcid update did:pplc:zQmfTxojQjja2gVvnuGEHyWZU6zQAdLLErMt3T8Wei7MfFX \
...
```bash
did_v1=`echo '{"state": "initial"}' | pplcid create --doc-pwd pwd1 --rev-pwd pwd2 --json-output`
echo '{"state": "new"}' | pplcid update $(echo $did_v1 | jq -r '.did') --simulate --old-doc-pwd pwd1 --old-rev-pwd pwd2 --doc-pwd pwd3 --rev-pwd pwd4 | jq '{identifier: .did_old, options: {log_revoke: .log_revoke_old, log_update: .log_update, log_terminate: .log_terminate}, didDocument: .doc}' | curl -H "Content-Type: application/json" -d @- -X POST http://localhost:3000/1.0/update
```
...
echo '{"identifier":"did:pplc:zQmPyjnkL52gQxBBhPuCFkf167MzpCKquqbu5Qt5ia2JGTr","secret":{"doc_pwd":"pwd1","rev_pwd":"pwd2"}}' | \
    curl -H "Content-Type: application/json" -d @- -X POST https://pplcid-registrar.data-container.net/1.0/deactivate
```
...
*Note:* with the `logs` option of the [PPLDID CLI](https://github.com/peopledata/pplcid/tree/main/cli) it is possible to show the the complete log trail for a deactivated DID:

```bash
pplcid logs did:pplc:zQmPyjnkL52gQxBBhPuCFkf167MzpCKquqbu5Qt5ia2JGTr
```
...
```bash
echo '{"short": "lived"}' | pplcid create --doc-pwd pwd1 --rev-pwd pwd2
```

Output: `did:pplc:zQmPbLw9vJzjNHHDNqpnK4rFjisUeXhLFvy7Apm6yWEPKt9`

The JSON structure from above to invoke `POST /1.0/deactivate` can then be generated by using the `--simulate` option of the [PPLDID CLI](https://github.com/peopledata/pplcid/tree/main/cli) with the following command:

```bash
pplcid revoke did:pplc:zQmfTxojQjja2gVvnuGEHyWZU6zQAdLLErMt3T8Wei7MfFX \
...
```bash
echo '{"short": "lived"}' | \
    pplcid create --doc-pwd pwd1 --rev-pwd pwd2 --json-output | \
